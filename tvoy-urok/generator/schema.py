...
class FgosMode(str, Enum):
    """Насколько жёстко дек привязан к федеральной программе.

    Учителю не всегда нужна привязка к ФРП: репетитору, кружку, авторскому
    курсу или уроку «для интереса» программа только мешает. Но фактическая
    достоверность нужна всегда, поэтому это не тумблер, а три режима.

    FULL  — требования ФРП идут в промпт генерации + отчёт о соответствии.
            Дефолт для школьного урока.
    CHECK — в генерацию не вмешиваемся (тема и логика — как задумал учитель),
            но отчёт показываем справочно: «по программе не раскрыто то-то».
            Ничего не блокирует.
    OFF   — программа не используется вообще. Проверки фактической
            достоверности (анахронизмы, границы эпохи) ОСТАЮТСЯ: они не про
            стандарт, а про то, чтобы не было вранья.
    """
...
class Meta(BaseModel):
    ...
    fgos_mode: FgosMode = Field(
        default=FgosMode.FULL,
        description="Привязка к федеральной программе: full | check | off",
    )
