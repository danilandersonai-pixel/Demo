...
- Пропуск: сфера ушла ниже игрока (y − player.y > max((радиус + 30) × heightFactor,
  радиус + радиус сферы) — через одно и то же время после линии корабля на любом экране, но
  никогда пока сфера ещё касается корабля), не собранная → `chain = 0`,
...
- Магнит (`magnetRadius(skin, upgrades)` > 0): сферы в радиусе ускоряются к кораблю,
  ...
  Радиус и тяга заданы для эталонной высоты: по Y поле × heightFactor (в мире — эллипс
  R × R·heightFactor, так его и рисует рендер), вертикальная тяга тоже × heightFactor,
  как скорости падения.
...
- `setPaused(true)` — плавно заглушить все звучащие голоса; `setSettings` — громкости и
  выключение звука. Щелчок-подтверждение включения звука играет, только если перед этим
  переключатель этой вкладки вызвал `armToggleConfirm()`: настройки, пришедшие из другой
  вкладки, применяются молча. Без `unlock()` все методы безопасно ничего не делают.
