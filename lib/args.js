...
function applyConfig(opts, config) {
  ...
  // Отдельное окно без адресной строки: это настройка панели, а не только
  // флаг. Иначе включить её мог бы лишь тот, кто ходит в терминал.
  if (!out.app && cfg.appWindow === true) out.app = true;
