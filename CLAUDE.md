...
- `shturman/` — **«Штурман»**, отдельное приложение: локальная панель-наставник,
  ...
  и звонит, когда тот остановился. Ставится и запускается двойным кликом,
  открывается с телефона по QR-коду, ставится как PWA. К свадебному сайту
  отношения не имеет.
...
│   │                                 lock/desktop/projects/risk — запуск кликом)
...
Главный путь у Штурмана без терминала: двойной клик по «Установить
Штурман», потом двойной клик по ярлыку на рабочем столе. Ниже — то, чем
проверяется код.
...
node server.js --pick    # с экраном выбора проекта (так работает ярлык)
```
...
Сервер для ручных проверок должен смотреть в папку с живым транскриптом
Claude Code, иначе лента и пульс будут пустыми и проверки упадут не по делу.
