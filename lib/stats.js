...
var attentionLib = require('./attention');
...
function createStats(options) {
  ...
  var recentFiles = [];        // плоский список — для компактного показа
  var RECENT_LIMIT = 12;
  var attention = attentionLib.createAttention({ now: now });   // он же в виде графа
  ...
  return {
      ...
      if (ev.kind === 'tool') {
        ...
        if (ev.action === 'edit' || ev.action === 'write') {
          ...
          if (ev.file) {
            ...
            noteRecent(ev.file, 'edit');
            attention.touch(ev.file, 'edit', ev.ts);
          }
        }
        if (ev.action === 'read') {
          reads++;
          if (ev.file) {
            ...
            noteRecent(ev.file, 'read');
            attention.touch(ev.file, 'read', ev.ts);
          }
        }
    ...
    snapshot: function (extra) {
      ...
      return {
        ...
        attention: attention.snapshot(t),
