...
function createDetector(options) {
  ...
  // Отдельный флаг, а не проверка lastActivityAt на истинность: метка
  // времени вполне может быть нулём (в тестах время отсчитывается от нуля).
  ...
  return {
    ...
    quietMs: function (now) {
      var t = now === undefined ? Date.now() : now;
      return seenActivity ? t - lastActivityAt : 0;
    },
