...
export function ScoreBlock({ score, best, beaten }: ScoreBlockProps) {
  return (
    <div className="flex min-w-0 flex-col items-start">
      <span className="text-[9px] font-bold tracking-[0.4em] text-neon-cyan/80 sm:text-[11px]">SCORE</span>
      <ScoreDigits
        value={score}
        className="text-[25px] font-extrabold leading-none tracking-wider sm:text-5xl sm:tracking-widest"
        digitsClassName={beaten ? 'text-neon-yellow text-glow-yellow' : 'text-white text-glow-cyan'}
      />
      <div className="mt-1.5 flex items-center gap-1.5 text-[10px] font-bold tracking-widest sm:mt-2 sm:gap-2 sm:text-xs">
        ...
        <AnimatePresence>
          {beaten && (
            <motion.span
              ...
              className="nv-record-flicker inline-flex items-center gap-1 rounded-[2px] bg-neon-yellow px-1.5 py-px text-[9px] font-black text-void shadow-[0_0_12px_rgba(255,233,74,0.75)] sm:text-[10px]"
            >
              <Crown size={10} strokeWidth={3} aria-hidden />
              NEW<span className="hidden sm:inline"> RECORD</span>
            </motion.span>
          )}
        </AnimatePresence>
      </div>
