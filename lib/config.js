...
var DEFAULTS = {
  ...
  appWindow: false,         // открывать отдельным окном без адресной строки
  density: 'cozy',          // cozy | compact
  // Настройки скорости. Хранятся целиком, чтобы переезжали между
  // браузерами вместе с остальными настройками.
  perf: {
    profile: 'auto',        // smooth | thrifty | weak | auto
    animations: true,
    heat: true,
    feedLimit: 1500,
    frameMs: 0,
...
function merge(raw) {
  ...
  if (typeof raw.appWindow === 'boolean') out.appWindow = raw.appWindow;
  if (raw.perf && typeof raw.perf === 'object') {
    var p = {};
    var PROFILES = ['smooth', 'thrifty', 'weak', 'auto'];
    p.profile = PROFILES.indexOf(raw.perf.profile) !== -1 ? raw.perf.profile : DEFAULTS.perf.profile;
    ['animations', 'heat', 'bigDiffs', 'highlight'].forEach(function (k) {
      p[k] = typeof raw.perf[k] === 'boolean' ? raw.perf[k] : DEFAULTS.perf[k];
    });
    p.feedLimit = [300, 600, 1500].indexOf(raw.perf.feedLimit) !== -1
      ? raw.perf.feedLimit : DEFAULTS.perf.feedLimit;
    p.frameMs = [0, 500, 1000].indexOf(raw.perf.frameMs) !== -1
      ? raw.perf.frameMs : DEFAULTS.perf.frameMs;
    out.perf = p;
  }
