...
└── yegor-diana-wedding/            — САМ САЙТ (публикуется в корень Pages)
    ...
    └── assets/
        ...
        ├── img/                    — локальные фото, дресс-код, QR, постер интро
        └── video/                  — интро-видео (intro.mp4 + intro.m4v)
```
...
- Медиа-ассеты ЛОКАЛЬНЫ: изображения в `assets/img/`, видео в `assets/video/`.
  `onerror`-хуки и CSS-плейсхолдеры/постер оставлены как страховка.
  `og:image`/`twitter:image` — АБСОЛЮТНЫЙ URL на GitHub Pages
  (`…github.io/Demo/assets/img/photo-01.jpg`): соцсети требуют абсолютный путь.
- Кэш-бастинг: `styles.css` и `main.js` подключаются с `?v=N` в `index.html`.
  При заметных правках стиля/скрипта бампать `N`, чтобы у гостей не залипал
  старый кэш.
