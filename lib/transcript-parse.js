...
function createParser(options) {
  ...
  function base(record, kind, action) {
    return {
      ...
      kind: kind,
      action: action,
      // Записи субагента лежат в том же файле и помечены isSidechain.
      // Без этого флага его чтения и правки перемешивались бы в ленте с
      // работой самого Клода, и было бы непонятно, кто что сделал.
