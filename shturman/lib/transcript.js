...
function createTailer(projectRoot, options) {
  ...
  // Первое определение уровня сообщается всегда, даже если он совпал с
  // начальным 'B': человек должен узнать, что транскриптов нет, а не гадать.
  var levelAnnounced = false;

  function setLevel(next, reason) {
    if (level === next && levelAnnounced) return;
    level = next;
    levelAnnounced = true;
  ...
  function switchTo(file, opts2) {
    ...
    // Сессия сменилась — наблюдать надо за новым файлом, старый больше не
    // растёт. Забыть перевесить значит вернуться к задержке опроса.
    watchActive();
  ...
  /**
   * Следить за текущим файлом транскрипта.
   *
   * fs.watch на некоторых системах и сетевых дисках молчит или врёт —
   * поэтому он именно ускоритель, а не замена опросу. Если он не завёлся
   * или отвалился, всё продолжает работать как раньше, просто медленнее.
   */
  var watcher = null;
  ...
  function watchActive() {
    ...
    try {
      watcher = fs.watch(activeFile, { persistent: false }, function () {
        // Событий файловой системы на одну дозапись приходит несколько;
        // склеиваем их в один разбор на кадр, иначе разбор пойдёт вхолостую.
        if (pumpQueued || stopped) return;
        pumpQueued = true;
        setTimeout(function () { pumpQueued = false; pump(); }, 16);
      });
      watcher.on('error', function () { unwatchActive(); });
    } catch (e) {
      watcher = null;                       // останется опрос
    }
  }

  function unwatchActive() {
    if (!watcher) return;
    try { watcher.close(); } catch (e) { /* уже закрыт */ }
    watcher = null;
  }
  ...
  return {
    ...
    start: function () {
      return findProjectDir(projectRoot).then(function (found) {
        ...
        // Опрос остаётся страховкой, но главным сигналом становится сама
        // файловая система: транскрипт дописывается в конец, и fs.watch
        // сообщает об этом сразу. С одним опросом задержка «Клод сделал →
        // видно на экране» была в среднем 324 мс и до 630 мс в худшем
        // случае — ровно половина периода опроса плюс разбор.
        watchActive();
