...
**PauseOverlay / GameOverOverlay** — `<Overlay blur>` + неоновая карточка, плавное появление.
...
лучший комбо, сферы, заработанные кристаллы, время, near miss), кошелёк, кнопки «Ещё раз»,
...
- **Leaderboard** — топ-5: место (1 — корона), счёт, уровень скорости (`LV 5 · ×1.56`), дата и
  время, лучший комбо, корабль. Подсветка последнего забега (`highlightId`). Пустое состояние.
